...
gulp.task("browsersync",function(){
	browsersync.init({
		...
		// },
		proxy:{
			//target:"http://gainextreme.local/order",
			target:"http://gainextreme.local/recharge?product=vpn-master"
		}
	});
});
...
gulp.task("fontspider",["fontbuild"],function(){});
	...
	return gulp.src(PATH.compileJsPath+"**/*.js")
		...
		.pipe(webpack({
			entry:{
				...
				recharge:PATH.compileJsPath+"ge.js",
				order:PATH.compileJsPath+"order.js",
			},
			output:{
      ...
      stats: {
		...
		// .pipe(gulp.dest(PATH.compileRevPath))
		.pipe(selfNotify({title:"Webpack",message:"Webpack task complete."}));
});

gulp.task("dev",["clean"],function(){
	runSequence(["less","fontspider","reactes6","image","libs:build","js"],"merge:js","watch","webpack","browsersync");
});
gulp.task("release",["clean"],function(){
	runSequence(["less","fontspider","reactes6","image","libs:build","js"],"merge:js","webpack");
});
...
// gulp.task("clean",function(){
// 	//return gulp.src([PATH.distJsPath+"**/*",PATH.distCssPath+"**/*.css",PATH.compileJsPath+"**/*.js",PATH.distImagePath+"**/*",PATH.compileRevPath+"**/*.json"],{read:false})
// 	return gulp.src([PATH.distJsPath+"**/*.js",PATH.distFontPath+"**",PATH.distCssPath+"**/*.css",PATH.compileJsPath+"**/*.js",PATH.compileRevPath+"**/*.json"],{read:false})
// 	.pipe(plumber({errorHandler:notify.onError("Clean Error:<%=error.message %>")}))
// 	.pipe(clean({force:true}))
// 	.pipe(selfNotify({
// 		title:"Clean Css,Js,Image",
...
// 	return gulp.src([PATH.compileRevPath+"**/*.json",PATH.distCssPath+"**/*.css"])
// 		.pipe(plumber({errorHandler:notify.onError("Rev CSS Error:<%=error.message %>")}))
// 		.pipe(revCollector({
// 			replaceReved:true,
// 		}))
// 		.pipe(gulp.dest(PATH.distCssPath))
// 		.pipe(selfNotify({title:"Images md5 in css",message:"Modify img md5 path in css task complete."}));
// });
// gulp.task("rev:html",function(){
// 	return gulp.src([PATH.compileRevPath+"**/*.json",PATH.sourceHtmlPath+"**/*.html"])
// 		.pipe(plumber({errorHandler:notify.onError("Rev Html Error:<%=error.message %>")}))
// 		.pipe(revCollector({
// 			replaceReved:true,
// 		}))
// 		.pipe(gulp.dest(PATH.sourceHtmlPath))
// 		.pipe(selfNotify({title:"Css and Js md5 in css",message:"Modify js and css md5 path in html task complete."}));
// });

// gulp.task("rev",["rev:html"]);
/*
gulp.task("es6:release",function(){
	return gulp.src(PATH.sourceJsPath+"**\/*.es6")
		.pipe(plumber({errorHandler:notify.onError("ES6 Error:<%=error.message %>")}))
		.pipe(babel({"presets":[es2015]}))
		.pipe(gulp.dest(PATH.compileJsPath))
		.pipe(webpack({
			output:{
				path:PATH.compileJsPath,
				filename:"./bundle_es6.js",
			},
			stats: {
			// Nice colored output
			colors: true
...
gulp.task("reactes6:release",["es6:release"],function(){
	return gulp.src(PATH.sourceJsPath+"jsx/*.jsx")
		.pipe(plumber({errorHandler:notify.onError("React ES6 Error:<%=error.message %>")}))
		.pipe(babel({"presets":[presetreact,es2015]}))
		.pipe(gulp.dest(PATH.compileJsPath))
